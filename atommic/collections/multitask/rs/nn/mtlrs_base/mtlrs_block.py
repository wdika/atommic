...
class MTLRSBlock(torch.nn.Module):
    ...
    def __init__(
        ...
        num_echoes: int = 1,  # TODO: check this
    ):
        ...
        self.num_echoes = num_echoes  # TODO: check this
    ...
    def forward(  # noqa: MC0001
        ...
        if self.consecutive_slices > 1 and self.reconstruction_module_dimensionality == 2:
            ...
            for slice_idx in range(self.consecutive_slices):
                ...
                if isinstance(hx, list):  # TODO: check this
                    if not hx[0].shape:
                        hx_slice = hx
                    else:
                ...
                for i, cascade in enumerate(self.reconstruction_module):
                    ...
                    prediction_slice, hx_slice = cascade(  # TODO: check this
                        prediction_slice,
                        y_slice,
                        sensitivity_maps_slice,
                        mask_slice,
                        _pred_reconstruction_slice if i == 0 else prediction_slice[-1],  # TODO: check this
                        hx_slice,  # TODO: check this
                        sigma,
                ...
                temp_hx.append(torch.stack(hx_slice, dim=0))  # TODO: check this
            preds = torch.stack(pred_reconstruction_slices, dim=3)
            hx = torch.stack(temp_hx, dim=2)  # TODO: check this
        ...
        if self.num_echoes > 1:  # TODO: check this
        ...
        if self.consecutive_slices > 1:
            ...
            # not be saved before
            pred_segmentation = pred_segmentation.view(  # TODO: check this
