...
class MRISegmentationUNetR(BaseMRISegmentationNet):
...
class CTSegmentationUNetR(BaseCTSegmentationNet):
    """Implementation of the UNETR for CT segmentation, as presented in [Hatamizadeh2022]_.
...
class SegmentationUNetR:
    """Factory class for the UNetR segmentation network, as presented in [Hatamizadeh2022]_.
    ...
    def get_model(cfg: DictConfig, trainer: Trainer = None):
        ...
        if modality == "mri":
            return MRISegmentationUNetR(cfg)
        elif modality == "ct":
            return CTSegmentationUNetR(cfg)
        else:
