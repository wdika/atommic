...
class MRISegmentation3DUNet(BaseMRISegmentationNet):
...
class CTSegmentation3DUNet(BaseCTSegmentationNet):
    """Implementation of the (3D) UNet for CT segmentation, as presented in [Ronneberger2015]_.
...
class Segmentation3DUNet:
    """Factory class for the (3D) UNet segmentation network, as presented in [Ronneberger2015]_.
    ...
    def get_model(cfg: DictConfig, trainer: Trainer = None):  # pylint: disable=unused-argument
        ...
        if modality == "mri":
            return MRISegmentation3DUNet(cfg)
        if modality == "ct":
            return CTSegmentation3DUNet(cfg)
